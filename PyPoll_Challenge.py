...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_list:
        ...
        print(f"{county}: {county_votes_percentage:.1f}% ({county_vote:,})\n")

         # 6e: Save the county votes to a text file.
        county_votes_result = f"{county}: {county_votes_percentage:.1f}% ({county_vote:,})\n"
        txt_file.write(county_votes_result) 

         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (county_vote > largest_county_turnout_count) and (county_votes_percentage > largest_county_percentage):
            # True
